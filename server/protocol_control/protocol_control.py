...
from server.ot_type import LabWare, Instrument, ModuleName, Mount, Target, Axis, Point, LabwareResult, PipetteResult
...
class ProtocolContext:
    ...
    async def _del_clear_a_run(self, run_id):
        ...
        _url = f"/runs/{run_id}"
        data = {}
        ret = HttpClient.delete(_url, data)
        HttpClient.judge_state_code(ret)
    ...
    async def _get_run(self):
        ...
        if self.run_id is None:
            raise ValueError("run id can't be None")
        ret = HttpClient.get(f'/runs/{self.run_id}')
        HttpClient.judge_state_code(ret)
        return ret[1]
    ...
    async def _post_load_labware(self, labware_name: LabWare, slot_name="9", name_space="opentrons", module_id=None):
        ...
        _params = {"waitUntilComplete": True}
        ret = HttpClient.post(_url, params=_params, data=_data)
        HttpClient.judge_state_code(ret)
        return ret[1]
    ...
    async def _get_command(self, run_id, length):
        ...
        ret = HttpClient.get(_url, params=_param)
        HttpClient.judge_state_code(ret)
        return ret[1]
    ...
    async def mix_liquid(self, pipette_id, labware_id, volume, times: int, well='A1'):
        ...
        for i in range(times):
            await self.aspirate(pipette_id, labware_id, volume, well=well)
            time.sleep(0.3)
            await self.dispense(pipette_id, labware_id, volume, well=well)