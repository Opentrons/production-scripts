<template>
  <div class="data-view">
    <el-card class="data-card">
      ...
      <div
        v-if="(collectionsLoading || dataLoading) && !collectionsError && !dataError && tableData.length === 0"
      ...
      <el-table 
        v-if="tableData.length > 0 && !collectionsError && !dataError"
        :data="tableData" 
      ...
      <template v-else>
        ...
        <div
          v-if="(unitTrackerLoading || unitTrackerSyncing) && unitTrackerRows.length === 0 && !unitTrackerError"
          class="records-loading-state"
        >
          <el-icon class="is-loading records-loading-icon"><Loading /></el-icon>
        ...
        <el-table
          ...
          v-loading="unitTrackerLoading || unitTrackerSyncing"
          stripe
          style="width: 100%"
...
import { collectionApi, uploadRecordApi } from '@/scripts/api'
...
const fetchCollections = async () => {
  ...
  try {
    ...
    const responseError = String(response.data.error || '').trim()
    if (responseError) {
      collections.value = []
      tableData.value = []
      total.value = 0
      collectionsError.value = responseError
      return
    }
    collections.value = sortCollections(response.data.collections || [])
    currentPage.value = 1
    const filterLoaded = await fetchFilterOptions()
    if (filterLoaded) {
