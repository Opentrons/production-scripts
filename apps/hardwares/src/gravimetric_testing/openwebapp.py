...
def prompt_rasp_no() -> str:
    return input("Please input raspberry pie No (请输入树莓派编号(GRAV1)):").strip()


def prompt_open_web() -> str:
    return input("Open data website? (是否打开称数据网站) [y/N]:").strip()
...
def openweb():
    json_file_path = resource_path("gravapp", "addr.json")
    name = prompt_rasp_no()
    use_key = prompt_open_web()
    try:    
