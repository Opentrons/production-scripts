...
P1000_96_SAMPLE_PATH = Path(__file__).resolve().parents[3] / "csv-samples" / "opentrons" / "P1000-CH96" / "P1KHV3620260603A01-qc - %D.csv"
P50S_VOLUME_SAMPLE_PATH = Path(__file__).resolve().parents[3] / "csv-samples" / "opentrons" / "P50S" / "P50SV3620260626A12-qc - Volume.csv"
P1KS_GRAV_SAMPLE_PATH = Path(__file__).resolve().parents[3] / "csv-samples" / "opentrons" / "P1KS" / "P1KSV3620260422A17-qc - Gravimetric Raw Data.csv"
P50M_GRAV_SAMPLE_PATH = Path(__file__).resolve().parents[3] / "csv-samples" / "opentrons" / "P50M" / "P50MV3520260425A03-qc - Gravimetric Raw Data.csv"
P1KM_GRAV_SAMPLE_PATH = Path(__file__).resolve().parents[3] / "csv-samples" / "opentrons" / "P1KM" / "P1KMV3520260626A02-qc - Gravimetric Raw Data.csv"
PIPETTE_ASSEMBLY_QC_SAMPLE_PATH = Path(__file__).resolve().parents[3] / "csv-samples" / "opentrons" / "P50S" / "P50SV3620240909A09-assembly-qc.csv"
P2HH_ASSEMBLY_QC_RAWDATA_SAMPLE_PATH = Path(__file__).resolve().parents[3] / "csv-samples" / "opentrons" / "P200-CH96" / "P2HHV3220260612A03-Assembly-QC-20260613105121 - RawData.csv"
ROBOT_ASSEMBLY_QC_SAMPLE_PATH = Path(__file__).resolve().parents[3] / "csv-samples" / "opentrons" / "Flex" / "robot-assembly-qc-ot3_run-26-06-04-08-56-54.csv"
...
def test_gravimetric_spec_json_fallback_overrides_default(tmp_path, monkeypatch):
    monkeypatch.setenv("PRODUCTION_PLATFORM_DATA_ANALYSIS_SPEC_JSON_PATH", str(tmp_path / "specs.json"))
