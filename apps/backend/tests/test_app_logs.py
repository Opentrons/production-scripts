from __future__ import annotations

import io
import zipfile

import pytest

from modules.robots import app_logs
from modules.robots.api_client.client import OpentronsApiError


class FakeResponse:
    def __init__(self, content: bytes):
        self.content = content


class FakeClient:
    def __init__(self, by_path=None, fail_paths=None):
        self._by_path = by_path or {}
        self._fail_paths = fail_paths or set()
        self.requests = []

    def request_raw(self, method, path, **kwargs):
        self.requests.append((method, path, kwargs))
        if path in self._fail_paths:
            raise OpentronsApiError(f"fetch failed: {path}")
        return FakeResponse(self._by_path.get(path, f"content of {path}".encode("utf-8")))


def test_collect_bundles_standard_log_endpoints(monkeypatch):
    client = FakeClient(
        by_path={
            "/logs/api.log": b"api log body",
            "/logs/server.log": b"server log body",
            "/logs/can_bus.log": b"can bus log body",
            "/logs/serial.log": b"serial log body",
            "/logs/touchscreen.log": b"touchscreen log body",
            "/logs/update_server.log": b"update server log body",
        },
    )
    monkeypatch.setattr(app_logs, "OpentronsHttpClient", lambda ip, port: client)

    zip_bytes, filename = app_logs.collect_opentrons_app_logs("192.168.6.123", 31950)

    assert filename.startswith("opentrons-app-logs-192.168.6.123-")
    assert filename.endswith(".zip")
    archive = zipfile.ZipFile(io.BytesIO(zip_bytes))
    names = archive.namelist()
    assert "opentrons-logs/api.log" in names
    assert "opentrons-logs/can_bus.log" in names
    assert "opentrons-logs/serial.log" in names
    assert "opentrons-logs/touchscreen.log" in names
    assert "opentrons-logs/update_server.log" in names
    assert "opentrons-logs/server.log" in names
    assert archive.read("opentrons-logs/api.log") == b"api log body"
    assert archive.read("opentrons-logs/can_bus.log") == b"can bus log body"
    assert archive.read("opentrons-logs/server.log") == b"server log body"
    assert {path for _, path, _ in client.requests} == {
        f"/logs/{identifier}" for identifier in app_logs.STANDARD_LOG_IDENTIFIERS
    }
    assert all(
        kwargs == {"timeout": app_logs.APP_LOG_REQUEST_TIMEOUT}
        for _, _, kwargs in client.requests
    )


def test_collect_skips_single_missing_log(monkeypatch):
    client = FakeClient(
        fail_paths={"/logs/server.log"},
    )
    monkeypatch.setattr(app_logs, "OpentronsHttpClient", lambda ip, port: client)

    zip_bytes, _ = app_logs.collect_opentrons_app_logs("192.168.6.123", 31950)

    archive = zipfile.ZipFile(io.BytesIO(zip_bytes))
    assert archive.namelist() == [
        "opentrons-logs/api.log",
        "opentrons-logs/can_bus.log",
        "opentrons-logs/serial.log",
        "opentrons-logs/touchscreen.log",
        "opentrons-logs/update_server.log",
    ]


def test_collect_raises_when_no_logs_available(monkeypatch):
    client = FakeClient()
    monkeypatch.setattr(app_logs, "OpentronsHttpClient", lambda ip, port: client)
    client._fail_paths.update({
        f"/logs/{identifier}" for identifier in app_logs.STANDARD_LOG_IDENTIFIERS
    })

    with pytest.raises(ValueError):
        app_logs.collect_opentrons_app_logs("192.168.6.123", 31950)
