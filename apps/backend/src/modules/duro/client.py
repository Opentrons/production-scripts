...
class DuroClient:
    ...
    def _access_token(self) -> str:
        ...
        if not token:
            raise DuroAuthenticationError(
                "未配置可用的 Duro 凭据，请设置 PRODUCTION_PLATFORM_DURO_API_KEY、PRODUCTION_PLATFORM_DURO_TOKEN、"
    ...
    def _raw_credential(self) -> str:
        ...
        # otherwise prefer the long-lived API key loaded from apps/backend/.env.
        value = os.getenv("PRODUCTION_PLATFORM_DURO_TOKEN", "").strip()
        if not value:
            value = os.getenv("PRODUCTION_PLATFORM_DURO_API_KEY", self.api_key).strip()
