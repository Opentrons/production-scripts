...
from core.config import ENVIRONMENT, EXPIRE_DAYS
from core.logging import get_logger

from modules.uploads.handler.models import FileDescription, TestTypes, UploadApiResponse, build_api_response
...
class UploadData:
    def __init__(self, mongo: MongoDB = None) -> None:
        ...
        from modules.uploads.handler.repositories.upload_session_repository import UploadSessionRepository
        from modules.uploads.handler.repositories.upload_repository import default_upload_repositories
...
def build_slack_failure_hints(
    ...
    *,
    ...
    if failure_stage == "数据库写入" or database_success is False:
        hints.append(
            "检查 MongoDB：`systemctl status mongod`，并查看 `/var/log/production-backend-error.log` 中的 `MongoDB 连接失败 - Host`。"
        )
    if failure_stage == "Google Drive / Sheets" or upload_success is False:
        hints.append(
            "检查 Google 鉴权与代理：确认 `/configs/token.json` 有效，且 `PRODUCTION_PLATFORM_GHELPER_DIR` 指向的目录完整。"
        )
    ...
    if "proxy" in lowered or "ghelper" in lowered:
        hints.append("检查 `PRODUCTION_PLATFORM_GHELPER_DIR`（默认 `apps/backend/ghelper-test`），必要时重新部署 API。")
    if "file is not finished" in lowered:
        hints.append("CSV 对应测试可能尚未完成，确认设备端测试状态后再上传。")

    if not hints:
        hints.append("查看 Web UI 上传记录详情，并在服务器检索 `production-backend-error.log` 获取完整堆栈。")
    return hints
