...
def build_tools() -> list[AgentTool]:
    ...
    return [
        ...
        AgentTool("get_platform_health", "读取平台后端、Slack 和 Google Drive 的缓存健康状态；仅在用户明确要求立即检查时使用 refresh=true 发起实时探测。", _object({"refresh": _boolean("是否立即重新探测外部服务", False)}), platform.get_platform_health),
        AgentTool("get_platform_version", "读取当前生产平台部署版本、更新时间和 Git commit。", _object({}), platform.get_platform_version),
        AgentTool("list_downloadable_resources", "列出资源下载模块中的项目、版本、文件信息和可点击 download_url。", _object({}), platform.list_downloadable_resources),
        AgentTool("query_robot_log_downloads", "查询 Robot Log 下载历史、任务状态和服务器文件可用性；可用文件会包含可点击 download_url。", _object({"ip": _string("设备 IPv4；空字符串返回全部设备", default=""), **pagination}), platform.query_robot_log_downloads),
        AgentTool("query_agent_schedules", "查询生产助手定时任务及最近执行结果。", _object({"include_runs": _boolean("是否包含最近执行记录", True), "limit": _integer("执行记录数量", 30, maximum=100)}), platform.query_agent_schedules),
        ...
        AgentTool("list_robot_testing_data", "浏览指定 Flex 机器人 /data/testing_data 下的测试数据文件和目录。path 只能位于测试数据根目录内。", _object({"ip": _string("设备 IPv4"), "path": _string("测试数据相对路径或完整路径；空字符串读取根目录", default="")}, ["ip"]), platform.list_robot_testing_data),
        AgentTool("prepare_robot_testing_data_download", "为用户已选择的机器人测试数据创建受登录保护的临时 ZIP 下载链接。调用前应先用 list_robot_testing_data 确认真实路径；返回 download_url 后必须在回答中原样输出为 Markdown 可点击链接。", _object({"ip": _string("设备 IPv4"), "paths": {"type": "array", "items": {"type": "string"}, "minItems": 1, "maxItems": 500, "description": "已确认下载的测试数据文件或目录路径"}}, ["ip", "paths"]), platform.prepare_robot_testing_data_download, "download"),
