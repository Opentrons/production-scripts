...
DEFAULT_AUTH_REFRESH_TOKEN_HOURS = 24 * 15
AUTH_REFRESH_TOKEN_HOURS = max(
    1,
    int(
        os.getenv(
            "PRODUCTION_PLATFORM_AUTH_REFRESH_TOKEN_HOURS",
            str(DEFAULT_AUTH_REFRESH_TOKEN_HOURS),
        )
    ),
)
...
ROBOT_APP_LOG_ANALYSIS_COLLECTION = "robot_app_log_analysis_records"
ROBOT_SSH_COMMAND_COLLECTION = "robot_ssh_commands"
ROBOT_VERSION_RECORD_COLLECTION = "robot_version_records"
ROBOT_VERSION_COMPARISON_RULE_COLLECTION = "robot_version_comparison_rules"
...
WORKFLOW_REFINE_BATCH_SIZE = max(
    1,
    int(os.getenv("PRODUCTION_PLATFORM_WORKFLOW_REFINE_BATCH_SIZE", "25")),
)
...
GOOGLE_SERVICE_ACCOUNT_PATH = Path(
    os.getenv("PRODUCTION_PLATFORM_GOOGLE_SERVICE_ACCOUNT_PATH", "").strip()
    or Path(GOOGLE_AUTH_DIR) / "service-account.json"
)
...
INFORMATION_REFRESH_SECONDS = max(
    60,
...
SOP_FULL_SEMANTIC_MAX_REFERENCES = max(
    1,
    int(os.getenv("PRODUCTION_PLATFORM_SOP_FULL_SEMANTIC_MAX_REFERENCES", "120")),
)
...
DURO_REQUEST_TIMEOUT_SECONDS = int(os.getenv("PRODUCTION_PLATFORM_DURO_TIMEOUT_SECONDS", "120"))
...
BRIDGEFLOODS_REQUEST_TIMEOUT_SECONDS = max(
    1.0,
    float(os.getenv("PRODUCTION_PLATFORM_BRIDGE_TIMEOUT_SECONDS", "60")),
)
