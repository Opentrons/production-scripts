...
- API 入口：`apps/backend/src/api/routers/uploads.py::upload_data(payload)`
- 业务入口：`apps/backend/src/modules/uploads/handler/upload.py::UploadData.update_data_to_google_drive(file_path, zip_file=None)`

## 总览流程图

```mermaid
flowchart TD
    A["POST /api/upload-data<br/>uploads.upload_data(payload)"] --> B["读取请求模型<br/>csv_file_path, zip_file_path"]
    B --> C["UploadData()"]
...
`uploads.upload_data(payload)` 从请求模型读取：
...
文件：`apps/backend/src/modules/uploads/handler/upload.py`
...
文件：`apps/backend/src/modules/uploads/handler/models/domain.py`
...
文件：`apps/backend/src/modules/uploads/handler/parsers/registry.py`
...
文件：`apps/backend/src/modules/uploads/handler/parsers/definitions.py`
...
文件：`apps/backend/src/modules/uploads/handler/parsers/csv_common.py`
...
文件：`apps/backend/src/modules/uploads/handler/product_catalog.py`
...
文件：`apps/backend/src/modules/uploads/handler/repositories/upload_repository.py`
...
文件：`apps/backend/src/modules/uploads/handler/uploaders/spreadsheet_uploader.py`
...
文件：`apps/backend/src/modules/uploads/handler/uploaders/workflows.py`
...
文件：`apps/backend/src/modules/uploads/handler/uploaders/common.py`
...
- Mongo host：默认跟随 `settings.PRODUCTION_PLATFORM_HOST`，本地开发为 `192.168.6.34`，服务器环境为 `localhost`。可用 `PRODUCTION_PLATFORM_LOCAL_SERVER_HOST`、`PRODUCTION_PLATFORM_MONGO_HOST` 覆盖。
...
文件：`apps/backend/src/modules/uploads/handler/repositories/upload_session_repository.py`
...
### `uploads.upload_data(payload)` 成功后

文件：`apps/backend/src/api/routers/uploads.py`
...
| --- | --- |
| `debug` | `apps/backend/src/modules/uploads/handler/configs/upload_debug.yaml` |
| `production` | `apps/backend/src/modules/uploads/handler/configs/upload_production.yaml` |
