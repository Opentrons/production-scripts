...
```text
production-backend.service - Data Handler FastAPI Service
     Loaded: loaded (/etc/systemd/system/production-backend.service; enabled; preset:>
 ...
 Invocation: a8e932f2a6fb4f3c8cd2c6fbc1d6ab08
   Main PID: 1412632 (uv)
     ...
     Memory: 11.1G (peak: 11.1G swap: 1.2G swap peak: 1.2G)
        CPU: 2h 7min 26.421s
     CGroup: /system.slice/production-backend.service
             ├─1412632 /root/.local/bin/uv run python -m uvicorn app:app --host>
             └─1412637 /opt/production-scripts/.venv
```
...
- `apps/backend/src/modules/uploads/handler/drivers/google_drive.py`
...
- `apps/backend/src/modules/robots/robots.py`
- `apps/backend/src/modules/uploads/upload.py`
...
- `apps/backend/src/modules/test_management/domain/execution/manager.py`
...
pytest apps/backend/tests/test_data_analysis.py -q
```
...
```bash
sudo systemctl restart production-backend.service
```
...
```bash
systemctl status production-backend.service
ps -L -p "$(systemctl show -p MainPID --value production-backend.service)" | wc -l
journalctl -u production-backend.service -n 200 --no-pager
```
