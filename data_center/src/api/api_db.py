...
from src.database.driver import MongoDBReader
import math
...
async def require_upload_switch(instance:ServiceHandler = Depends(get_instance)):
    try:
        await instance.init_database()
        result = instance.database_manager.params.auto_upload
        return JSONResponse(
            status_code=200,
            content={
                "status": result
...
async def require_upload_switch(value: bool, instance:ServiceHandler = Depends(get_instance)):
    try:
        instance.database_manager.params.auto_upload = value
        return JSONResponse(
            status_code=200,
            content={
                "status": instance.database_manager.params.auto_upload
            }
        )
    except Exception as e:
        return HTTPException(status_code=400, detail=e)
