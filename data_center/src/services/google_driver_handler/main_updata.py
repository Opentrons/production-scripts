...
class updata_class():
    def __init__(self, test_environment: UploadEnv) -> None:
        """
            ...
            if uptemp == "PASS":
                ...
                for cs, pase in enumerate(u["ifpaste"]):
                    if pase["enabled"]:
                        ...
                        ret1 = self.gdrive.update_excel_sheet_page_batch(spreadsheet_id=paseexcelid,
                                                                         sheet_name=pasesheetname, ranges=rangeval,
                                                                         new_values=pasedata[0])
                        ret2 = self.gdrive.update_excel_sheet_page_batch(spreadsheet_id=paseexcelid,
                                                                         sheet_name=pasesheetname, ranges=NOTES,
                                                                         new_values=[[Note_str]])
                        if ret1:
                            pasestate = True
                        # 进度
                ...
                for cop in u["ifcopydata"]:
                    if cop["enabled"]:
                        if csv_link != None:
                            ...
                            testpass = \
                                self.gdrive.get_excel_sheet_page(copyExcelID, stname, "ROWS", testpassrange)[0][0][0]
                ...
                for cs, pase in enumerate(u["ifpaste"]):
                    if pase["enabled"]:
                        if csv_link != None:
                            ...
                            pasedata[0][0].insert(0, sheetlink)
                            # logger.info(pasedata)
                            copytestdata = self.gdrive.update_excel_sheet_page_batch(spreadsheet_id=paseexcelid,
                                                                                     sheet_name=pasesheetname,
                                                                                     ranges=rangeval,
                                                                                     new_values=pasedata[0])
                            if copytestdata:
                                allgid = self.gdrive.get_sheet_gid_map(paseexcelid)
                    ...
                    if cop["enabled"]:
                        if csv_link != None:
                            ...
                            if pipettetype in List_1ch:
                                testpass = \
                                    self.gdrive.get_excel_sheet_page(copyExcelID, stname, "ROWS", ["!AQ11:AQ11"])[0][0][
                                        0]
                            elif pipettetype in List_8ch:
                                testpass = \
                                    self.gdrive.get_excel_sheet_page(copyExcelID, stname, "ROWS", ["!BO11:BO11"])[0][0][
                                        0]
                            elif pipettetype == "P1000M Ultima":
                                testpass = \
                                    self.gdrive.get_excel_sheet_page(copyExcelID, stname, "ROWS", ["!BY11:BYO11"])[0][
                                        0][0]
                            testall = copydata
                            copydatalist.append(copydata)
                            if func_callback != None:
    ...
    def update_data_to_google_drive(self, upfile_path, pipette_sn, pipette_type, zip_file, test_type,
        ...
        try:
            if test_type == "pipette-assembly-qc-ot3":
                ...
                test_res = self.updatavolume_1CH_8CH(upfile_path, pipette_sn, pipette_type, zip_file,
                                                     func_callback=func_callback, Note_str=Note_str)
