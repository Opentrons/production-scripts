...
def updata_Excel():
    ...
    for i, u in enumerate(yamldata["tabledataupdate"]):
        ...
        for i, ii in enumerate(exdata):
            ...
            shedrive.updata_excel_sheel_page(spreadsheet_id="17Y6-PDvR9NcVjj-9gegbej7yuEtmDftIMBCP3zX3oVs",
                                             range_name="Pip1", range=rangel, new_values=ii)
...
def updata_Excel_1000(u):
    ...
    for excelid in spreadsheet_idlist:
        ...
        for iii, sheetname in enumerate(updatasheet):
            ...
            getret = shedrive.updata_excel_sheel_page_list(spreadsheet_id=excelid, range_name=sheetname,
                                                           rangelist=allrangelist, new_values=alldatalist)
            if getret:
                print("更新文件:成功", u)
            else:
                print("更新文件:失败", u)
    ...
    for cs, pase in enumerate(u["ifpaste"]):
        if pase["enabled"]:
            ...
            shedrive.updata_excel_sheel_page_list(spreadsheet_id=paseexcelid, range_name=pasesheetname,
                                                  rangelist=rangeval, new_values=pasedata)
...
def updatavolume_1CH(u):
    ...
    for f in allfail:
        ...
        if fz:
            ...
            getret = shedrive.updata_excel_sheel_page_list(spreadsheet_id=updatafileid, range_name=sheetname,
                                                           rangelist=allrangelist, new_values=alldatalist)
            if getret:
                print("更新文件:成功", u)
            else:
        ...
        for cs, pase in enumerate(u["ifpaste"]):
            if pase["enabled"]:
                ...
                print(pasedata)
                shedrive.updata_excel_sheel_page(spreadsheet_id=paseexcelid, range_name=pasesheetname, range=rangeval,
