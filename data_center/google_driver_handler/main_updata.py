...
class updata_class():
    ...
    def updatavolume_1CH(self,upfilepath,pipettesn,pipettetype,upfailelist):
        ...
        """
        #更新容量数据demo
        if isinstance(self.yamldata, dict) and "1ch_updata_volume" in self.yamldata and isinstance(self.yamldata["1ch_updata_volume"], list):
        ...
        if u["explanation"] == "VOLUME1CH":
            if u["ifupdata"]:
                ...
                if fz:
                    ...
                    getret = self.shedrive.updata_excel_sheel_page_list(spreadsheet_id=updatafileid,range_name=sheetname,rangelist=allrangelist,new_values=alldatalist)
                    if getret:
                        print("更新文件:成功",u)
                    else:
                        print("更新文件:失败",u)
                ...
                for cs,pase in enumerate(u["ifpaste"]):
                    if pase["off/on"]:
                        ...
                        print(pasedata)
                        self.shedrive.updata_excel_sheel_page(spreadsheet_id=paseexcelid,range_name=pasesheetname,range=rangeval,new_values=pasedata[0])
...
if __name__ == "__main__":
    ...
    aa = updata_class()
    aa.star_int()
    aa.updatavolume_1CH("/Users/yew/Desktop/production-scripts/data_center/google_driver_handler/gravimetric-ot3-p1000-single_run-25-07-11-17-58-22_CSVReport-P1KSV3520230727A04-qc.csv","P1KSV3520230727A04","P1000S",["/Users/yew/Desktop/production-scripts/data_center/google_driver_handler/gravimetric-ot3-p1000-single_run-25-07-11-17-58-22_CSVReport-P1KSV3520230727A04-qc.csv"])
    # #更新数据
    # shedrive = None
    # shedrive = sheetdrive()
    # gdrive = googledrive()
