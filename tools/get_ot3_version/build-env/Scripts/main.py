...
test_name_list = {
    ...
    Production.Pipette: pipette_test_name_list,
}
...
def get_version(_test_name_list, production:Production):
    ...
    for key, value in versions.items():
        print(f"{key}: {value}")
    # save to csv
    if production == Production.Robot:
        csv_name = "VERSION_OT3.csv"
    elif production == Production.Pipette:
        csv_name = "VERSION_PIPETTE.csv"
    elif production == Production.Gripper:
        csv_name = "VERSION_GRIPPER.csv"
    else:
        raise Exception("<UNK>")
    try:
        ...
        while True:
            try:
                ...
                if choice in selection_dict:
                    ...
                    get_version(test_name_list[production], production)
                else:
                    print("无效的选择!")
            except ValueError:
                print("请输入数字!")
