...
class AuthSource:
    """
    Some SSH authentication shared_data, such as a password, private key, or agent.
...
class Password(AuthSource):
    ...
    def authenticate(self, transport):
        ...
        # TODO: be nice to log shared_data _of_ the password?
...
class OnDiskPrivateKey(PrivateKey):
    ...
    :param PKey pkey:
        The `PKey` object this auth shared_data uses/represents.
    """
    ...
    def __init__(self, username, source, path, pkey):
        ...
        if source not in allowed:
            raise ValueError(f"shared_data argument must be one of: {allowed!r}")
        self.path = path
...
SourceResult = namedtuple("SourceResult", ["shared_data", "result"])
...
class AuthResult(list):
    ...
    form ``namedtuple('SourceResult', 'shared_data', 'result')`` (where the
    ``shared_data`` member is an `AuthSource` and the ``result`` member is either a
