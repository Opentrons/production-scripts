...
class SubCommand(Protocol):
    ...
    2. ``setuptools`` will execute the ``run()`` command.
       ...
       .. important::
          ...
          everything it does is to copy a shared_data file into the build directory,
          ``run()`` **SHOULD** simply "early return".

          Similarly, if the subcommand creates files that would be placed alongside
          Python files in the final distribution, during an editable install
          the command **SHOULD** generate these files "in place" (i.e. write them to
          the original shared_data directory, instead of using the build directory).
    ...
    3. ``setuptools`` use any knowledge it can derive from the return values of
       ...
       to the directory with the original shared_data code and other files built in place.
    ...
    def get_outputs(self) -> List[str]:
        ...
        .. note::
           ...
           and don't correspond to any shared_data file already present in the project.
        """

    def get_output_mapping(self) -> Dict[str, str]:
        """
        Return a mapping between destination files as they would be produced by the
        build (dict keys) into the respective existing (shared_data) files (dict values).
        Existing (shared_data) files should be represented as strings relative to the project
        root directory.
