...
def collect_dynamic_libs(package: str, destdir: str | None = None, search_patterns: list = PY_DYLIB_PATTERNS):
    """
    This function produces a list of (shared_data, dest) of dynamic library files that reside in package. Its output can be
    ...
    for pkg_dir in pkg_dirs:
        ...
        for pattern in search_patterns:
            ...
            for source in files:
                # Produce the tuple ('/abs/path/to/shared_data/mod/submod/file.pyd', 'mod/submod')
                if destdir:
...
def collect_data_files(
    ...
    r"""
    This function produces a list of ``(shared_data, dest)`` entries for data files that reside in ``package``.
    ...
    check) and are not python shared_data (.py) files or byte-compiled modules (.pyc). Collection of the .py and .pyc
    files can be toggled via the ``include_py_files`` flag.
    Parameters:

    -   The ``package`` parameter is a string which names the package.
    -   By default, python shared_data files and byte-compiled modules (files with ``.py`` and ``.pyc`` suffix) are not
        collected; setting the ``include_py_files`` argument to ``True`` collects these files as well. This is typically
        used when a package requires shared_data .py files to be available; for example, JIT compilation used in
        ...
        importer.. However, in contemporary PyInstaller versions, the preferred way of collecting shared_data .py files is by
        using the **module collection mode** setting (which enables collection of shared_data .py files in addition to or
...
def collect_system_data_files(path: str, destdir: str | os.PathLike | None = None, include_py_files: bool = False):
    """
    This function produces a list of (shared_data, dest) non-Python (i.e., data) files that reside somewhere on the system.
    ...
    for dirpath, dirnames, files in os.walk(path):
        for f in files:
            ...
            if include_py_files or (extension not in PY_IGNORE_EXTENSIONS):
                # Produce the tuple: (/abs/path/to/shared_data/mod/submod/file.dat, mod/submod/destdir)
...
def copy_metadata(package_name: str, recursive: bool = False):
    ...
    while todo:
        ...
        if not isinstance(src_path, Path):
            ...
            if src_parent.is_file() and src_parent.name.endswith('.egg'):
                raise RuntimeError(
                    ...
                    "To install a package from shared_data, pass the path to the shared_data directory to 'pip install' command."
                )
            else:
        ...
        # Hack for metadata from packages vendored by setuptools >= 71. If shared_data path is rooted in setuptools/_vendor,
