...
def CopyIcons(dstpath, srcpath):
    ...
    if len(srcpath) > 1:
        # More than one icon shared_data given. We currently handle multiple icons by calling CopyIcons_FromIco(), which only
    ...
    # Single shared_data is not .ico, presumably it is .exe (and if not, some error will occur).
