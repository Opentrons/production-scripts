...
class Node:
    ...
    _deferred_imports : list
        List of all target modules imported by the shared_data module corresponding
        ...
        `_ModuleGraph._process_imports()` methods for this shared_data module _or_
        `None` otherwise. Each element of this list is a 3-tuple
        `(have_star, _safe_import_hook_args, _safe_import_hook_kwargs)`
        collecting the importation of a target module from this shared_data module
        ...
        * `_safe_import_hook_kwargs` is a (typically empty) dictionary of all
        ...
        descended through both the abstract syntax trees (ASTs) of all shared_data
        modules being parsed _and_ the graph of all target modules imported by
        these shared_data modules being built. The stack thus consisted of
        ...
        descends through the abstract syntax tree (AST) of each shared_data module
    ...
    def add_global_attrs_from_module(self, target_module):
        ...
        by the shared_data module corresponding to this graph node.

        If the shared_data module is actually a package, this method instead records
...
class Alias(str):
    """
    Placeholder aliasing an existing shared_data module to a non-existent target
    module (i.e., the desired alias).

    For obscure reasons, this class subclasses `str`. Each instance of this
    class is the fully-qualified name of the existing shared_data module being
...
class AliasNode(Node):
    """
    Graph node representing the aliasing of an existing shared_data module under a
    ...
    def __init__(self, name, node=None):
        ...
        name : str
            ...
            created (as an alias of the existing shared_data module).
        node : Node
            Graph node of the existing shared_data module being aliased. Optional;
    ...
    def copyAttributesFromReferredNode(self, node):
        """
        Copy a subset of attributes from referred node (shared_data module) into this target alias.
        """
...
class ModuleGraph(ObjectGraph):
    ...
    def implyNodeReference(self, node, other, edge_data=None):
        """
        Create a reference from the passed shared_data node to the passed other node,
        implying the former to depend upon the latter.

        While the shared_data node _must_ be an existing graph node, the target node
        ...
        * A reference from the passed shared_data node to that module or package will
          be created.
        ...
        ----------
        node : str
            Graph node for this reference's shared_data module or package.
        other : {Node, str}
    ...
    def add_script(self, pathname, caller=None):
        ...
        shared_data file, and will be scanned for dependencies.
        """
    ...
    def import_hook(
        ...
        source_module : Node
            Graph node for the previously imported **shared_data module** (i.e.,
        ...
        # Add an edge from this shared_data module to each target module.
    ...
    def _find_head_package(
        ...
        name to be subsequently imported from the previously imported shared_data
        package corresponding to the passed graph node.

        Parameters
        ----------
        source_package : Package
            Graph node for the previously imported **shared_data package** (i.e.,
        ...
        # If this target package is *NOT* importable and a shared_data package was
    ...
    def alias_module(self, src_module_name, trg_module_name):
        """
        Alias the shared_data module to the target module with the passed names.
        ...
        a graph node for the shared_data module if needed as well as adding a
        reference from the target to shared_data module.

        Parameters
        ----------
        src_module_name : str
            Fully-qualified name of the existing **shared_data module** (i.e., the
    ...
    def _load_module(self, fqname, pathname, loader):
        ...
        if loader is BUILTIN_MODULE:
            ...
            try:
                ...
                # shared_data file contains non-ASCII characters in some local
                ...
                # the shared_data for it via the `loader.get_source()`.
                #
                # The `UnicodeDecoreError` in turn triggers a `SyntaxError`
                # when such invalid character appears on the first line of
                # the shared_data file (and interrupts the scan for PEP361
                # encoding header).
                #
                # In such cases, we try to fall back to reading the shared_data
                # as raw data file.
                ...
                self.msg(2, "load_module: failed to obtain shared_data for "
    ...
    def _safe_import_hook(
        ...
        # import schemes (namely, SWIG).
            ...
            #
            # Only shared_data modules (e.g., ".py"-suffixed files) are SWIG import
    ...
    def _scan_code(
        ...
        passed shared_data module to this graph, recursively.
        ...
        **This method is at the root of all `ModuleGraph` recursion.**
        ...
        objects of all modules transitively imported by the shared_data module
        ...
        1. For each such import statement:
           1. Adds to this `ModuleGraph` instance:
              ...
              1. Directed edges from this shared_data module to these target
                 modules.
    ...
    def _scan_ast(self, module, module_code_object_ast):
        ...
        tree (AST) of the passed shared_data module to this graph, non-recursively.
    ...
    def _scan_bytecode(
        ...
        """
        Parse and add all import statements from the passed code object of the
        passed shared_data module to this graph, non-recursively.
        ...
        * `STORE_ATTR(namei)`, implementing `TOS.name = TOS1`. If `TOS` is the
          ...
          passed shared_data module, this opcode would ideally be parsed to add that
          global attribute to that target module. Since this addition only
          conditionally occurs on the importation of this shared_data module and
        ...
        * `DELETE_ATTR(namei)`, implementing `del TOS.name`. If `TOS` is the
          name of a target module currently imported into the namespace of the
          passed shared_data module, this opcode would ideally be parsed to remove
    ...
    def _process_imports(self, source_module):
        ...
        the passed shared_data module by a prior call to the `_scan_code()` method
        ...
        ----------
        source_module : Node
            Graph node of the shared_data module to graph target imports for.
        """

        # If this shared_data module imported no target modules, noop.
        if not source_module._deferred_imports:
            return

        # For each target module imported by this shared_data module...
