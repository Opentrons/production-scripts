...
def toc_process_symbolic_links(toc):
    ...
    for entry in toc:
        ...
        # Skip entries without valid shared_data name (e.g., OPTION)
        if not src_name:
            new_toc.append(entry)
            continue
...
def _try_preserving_symbolic_link(dest_name, src_name, all_dest_files):
    ...
    while True:
        ...
        # Stop when referenced shared_data file is not a symbolic link anymore.
        ...
        if ref_dest_file in all_dest_files:
            # Sanity check: original shared_data name and current referenced shared_data name must, after complete resolution,
