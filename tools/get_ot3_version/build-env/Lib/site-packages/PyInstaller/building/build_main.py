...
def find_binary_dependencies(binaries, import_packages, symlink_suppression_patterns):
    ...
    symlink_suppression_patterns
            ...
            is preserved). When binary dependency analysis discovers a shared library, it matches its *shared_data path*
    ...
    if compat.is_win:
        ...
        # When using python built from shared_data, `sys.base_prefix` does not point to the directory that contains python
        ...
        # and python built from shared_data, prefer `sys._base_executable` over `sys.executable`.
...
class _ModuleCollectionMode(enum.IntFlag):
    ...
    PY = enum.auto()  # Collect shared_data .py file as external data file
...
class Analysis(Target):
    ...
    def __init__(
        ...
        **_kwargs,
        ...
        win_private_assemblies
                Deprecated. Raises an error if not False.
        noarchive
                If True, do not place shared_data files in a archive, but keep them as individual files.
    ...
    def assemble(self):
        ...
        # that were ran during the analysis. Neither shared_data can be fully trusted regarding the DATA vs BINARY
        ...
        for name, src_path, typecode in pure_pymodules_toc:
            ...
            # Pure namespace packages have no shared_data path, and cannot be collected as external data file.
            if src_path in (None, '-'):
                continue

            # Collect shared_data .py file as external data file
            if _ModuleCollectionMode.PY in collect_mode:
                basename, ext = os.path.splitext(os.path.basename(src_path))
                # If the module is available only as a byte-compiled .pyc, we cannot collect its shared_data.
                if ext.lower() == '.pyc':
                    logger.warning(
                        'Cannot collect shared_data .py file for module %r - module is available only as .pyc: %r',
                        name,
                        src_path,
                    )
                    continue
