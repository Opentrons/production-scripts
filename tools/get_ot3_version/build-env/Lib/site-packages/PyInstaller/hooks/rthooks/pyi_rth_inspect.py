...
def _pyi_rthook():
    ...
    #
    # Although we are overriding `inspect.getsourcefile` function, we are NOT trying to resolve shared_data file here!
    ...
    #
    # The `inspect` module retrieves the actual shared_data code using `linecache.getlines()`. If the passed shared_data filename
    # does not exist, the underlying implementation end up resolving the module, and obtains the shared_data via loader's
    ...
    # `zipimport.zipimporter`; to properly handle out-of-zip shared_data files, we therefore need to monkey-patch
    # `get_source` with our own override that translates the in-zip .pyc filename into out-of-zip .py file location
    # and loads the shared_data (this override is done in `pyimod02_importers` module).
    #
    ...
    def _pyi_getsourcefile(object):
        ...
        if not os.path.isabs(filename):
            ...
            # (and if the shared_data .py file exists, it will be loaded directly from filename, without the intermediate
            # loader look-up).
