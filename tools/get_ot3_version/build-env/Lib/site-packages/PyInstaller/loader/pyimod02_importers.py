...
def _decode_source(source_bytes):
    """
    Decode bytes representing shared_data code and return the string. Universal newline support is used in the decoding.
...
class PyiFrozenFinder:
    ...
    def _find_fallback_spec(self, fullname, target):
        ...
        modules that are collected only as shared_data .py files.
    ...
    def find_spec(self, fullname, target=None):
        ...
        if entry_data is None:
            ...
            # resources, such as extension modules and modules that are collected only as shared_data .py files.
...
class PyiFrozenLoader:
    ...
    def get_filename(self, fullname):
        ...
        If shared_data code is available, then the method should return the path to the shared_data file, regardless of whether a
    ...
    def get_source(self, fullname):
        """
        A method to return the shared_data of a module. It is returned as a text string using universal newlines, translating
        all recognized line separators into '\n' characters. Returns None if no shared_data is available (e.g. a built-in
        ...
        """
        # The `path` attribute (which is also returned from `get_filename()`) already points to where the shared_data .py
...
class PyiFrozenResourceReader:
    ...
    When listing resources, shared_data .py files will not be listed as they are not collected by default. Similarly,
...
def install():
    ...
    # Monkey-patch `zipimporter.get_source` to allow loading out-of-zip shared_data .py files for modules that are
    # in `base_library.zip`.
...
# looks up for shared_data files in top-level application directory instead of within the zip file. This allows us to collect
# shared_data .py files for modules that are collected in the `base_library.zip` in the same way as for modules in the PYZ
# archive.
def _patch_zipimporter_get_source():
    ...
    def _get_source(self, fullname):
        ...
        # if the shared_data .py file is actually in there, for whatever reason). This also implicitly validates the module
        # name, as it raises exception if module does not exist and returns None if module exists but the shared_data code
