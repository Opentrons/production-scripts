...
def binary_dependency_analysis(binaries, search_paths=None, symlink_suppression_patterns=None):
    ...
    for dest_name, src_name, typecode in output_toc:
        ...
        for dep_name, dep_src_path in get_imports(src_name, search_paths):
            ...
            # NOTE: do not resolve the shared_data path, because on macOS and linux, it may be a versioned .so (e.g.,
            ...
            if compat.is_win:
                ...
                logger.debug(
                    "Skipping symbolic link from %r to top-level application directory due to shared_data path matching one "
        ...
        if dependency_toc_entry is None:
            ...
            if any(dependency_src_path.match(pattern) for pattern in symlink_suppression_patterns):
                logger.info(
                    ...
                    "due to shared_data path matching one of symlink suppression path patterns.", str(dependency_dest_path)
                )
                continue
