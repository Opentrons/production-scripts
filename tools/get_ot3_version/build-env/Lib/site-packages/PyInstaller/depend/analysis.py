...
class PyiModuleGraph(ModuleGraph):
    ...
    def _safe_import_hook(
        ...
        # referring (shared_data) modules and their parent package(s) are loaded by the exclusion mechanism that takes place
    ...
    def _metadata_from(self, package, methods=(), recursive_methods=()) -> set:
        ...
        Args:
            package:
                The module name that must be imported in a shared_data file to trigger the search.
            methods:
        ...
        Returns:
            Required metadata in hook data ``(shared_data, dest)`` format as returned by
            :func:`PyInstaller.utils.hooks.copy_metadata()`.

        Scan all shared_data code to be included for usage of particular *key* functions which imply that that code will
