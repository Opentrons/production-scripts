...
# triton has a JIT module that requires its shared_data .py files. For some god-forsaken reason, this JIT module
# (`triton.runtime.jit` attempts to directly read the contents of file pointed to by its `__file__` attribute (assuming
# it is a shared_data file). Therefore, `triton.runtime.jit` must not be collected into PYZ. Same goes for `compiler` and
