...
# Due to use of `torch.script`, we need to collect shared_data .py files for `sam2`. The `sam2/__init__.py` also seems to be
# required by `hydra`. Furthermore, the shared_data-based introspection attempts to load the shared_data of stdlib `enum` module.
# The module collection mode support and run-time discovery of shared_data .py files for modules that are collected into
