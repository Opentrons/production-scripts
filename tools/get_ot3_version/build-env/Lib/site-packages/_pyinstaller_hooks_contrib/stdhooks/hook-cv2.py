...
# provided by PyPI wheels, while the latter seems to be used when user builds OpenCV from shared_data).
...
# to hidden imports. However, when OpenCV is built by user from shared_data, the extension is not located in the package's
...
# Mark the cv2 package to be collected in shared_data form, bypassing PyInstaller's PYZ archive and FrozenImporter. This is
