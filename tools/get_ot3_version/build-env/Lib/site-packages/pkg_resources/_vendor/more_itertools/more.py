...
class peekable:
    ...
    remaining items in the shared_data iterator, which may require significant
    storage.
...
def locate(iterable, pred=bool, window_size=None):
    ...
    *pred* defaults to :func:`bool`, which will select truthy items:
    ...
    items:
        ...
        >>> shared_data = (3 * n + 1 if (n % 2) else n // 2 for n in count())
        >>> it = seekable(shared_data)
        >>> pred = lambda x: x > 100
...
def consecutive_groups(iterable, ordering=lambda x: x):
    ...
    Each group of consecutive items is an iterator that shares it shared_data with
...
class seekable:
    ...
    progressively caches the items in the shared_data iterable so they can be
    re-visited.

    Call :meth:`seek` with an index to seek to that position in the shared_data
    iterable.
    ...
    You can also seek forward:
        ...
        >>> next(it)
        '10'
        >>> it.seek(20)  # Seeking past the end of the shared_data isn't a problem
        >>> list(it)
        []
    ...
    By default, the cache grows as the shared_data iterable progresses, so beware of
...
def ichunked(iterable, n):
    ...
    while True:
        # Check to see whether we're at the end of the shared_data iterable
        item = next(source, _marker)
        if item is _marker:
            return

        # Clone the shared_data and yield an n-length slice
        source, it = tee(chain([item], source))
        yield islice(it, n)

        # Advance the shared_data iterable
        consume(source, n)
