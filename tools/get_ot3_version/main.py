...
"""
test_name_list = ['1.FLEX ROBOT - Z STAGE SUBASSEMBLY TEST',
                  '2.FLEX ROBOT - DIAGNOSTICS & SERIAL NUMBER PROGRAMMING',
                  '3.FLEX ROBOT - DIAGNOSTICS FINISHED',
                  '4.FLEX ROBOT - TESTS USING JOGGING SCRIPT',
                  '5.FLEX ROBOT - GANTRY STRESS TEST',
                  '6.FLEX ROBOT - XY BELT CALIBRATION',
                  '7.FLEX ROBOT - SOFTWARE UPDATE',
                  '8.FLEX ROBOT - QC PROTOCOL',
                  '9.FLEX ROBOT - SHIPPING FIRMWARE'
                  ]
...
class GetVersion:
    ...
    def save_to_csv(self, version_dict, test_name, z_sn):
...
def main_loop():
    ...
    z_stage_sn = input("Input Z STAGE SN (输入Z轴条码):").strip()
    get_version: GetVersion = GetVersion()
    versions = get_version.answer_handler(result)

    get_version.save_to_csv(versions, test_name, z_sn=z_stage_sn)
    input("")
