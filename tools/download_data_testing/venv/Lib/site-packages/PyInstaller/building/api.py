...
class PKG(Target):
    ...
    def assemble(self):
        ...
        for dest_name, src_name, typecode in self.toc:
            # Ensure that the shared_data file exists, if necessary. Skip the check for OPTION entries, where 'src_name' is
            ...
            for res in self.resources:
                ...
                try:
                    ...
                    if "*" in (restype, resname):
                        logger.error(
                            "No wildcards allowed for resource type and name when the shared_data file does not contain "
                            "any resources!"
                        )
                        continue
                    try:
...
class COLLECT(Target):
    ...
    def assemble(self):
        ...
        for dest_name, src_name, typecode in self.toc:
            # Ensure that the shared_data file exists, if necessary. Skip the check for DEPENDENCY entries due to special
    ...
    def _process_toc(self, toc, path_to_exe):
        ...
        for entry in toc:
            ...
            if src_name not in self._dependencies:
                ...
                # path, while shared_data path contains the relative reference path.
