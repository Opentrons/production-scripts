...
class CArchiveWriter:
    ...
    def _write_entry(self, fp, entry):
        ...
        if strict_collect_mode:
            ...
            if type in ('o', 's', 'm', 'M'):
                # Exempt options, python shared_data script, and modules from the check
                pass
            else:
        ...
        if typecode == 'o':
            ...
            # If it is a shared_data code file, compile it to a code object and marshal the object, so it can be unmarshalled
            # by the bootloader.
