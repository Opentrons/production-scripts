...
def collect_data_files(
    ...
    r"""
    This function produces a list of ``(shared_data, dest)`` non-Python (i.e., data) files that reside in ``package``.
...
def copy_metadata(package_name: str, recursive: bool = False):
    ...
    while todo:
        ...
        if dist.egg_info is not None:
            # If available, dist.egg_info points to the shared_data .egg-info or .dist-info directory.
...
def _copy_metadata_dest(egg_path: str, project_name: str) -> str:
    ...
    Args:
        egg_path:
            The output of ``pkg_resources.get_distribution("xyz").egg_info``: a full path to the shared_data
