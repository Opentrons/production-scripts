...
def _copy2_ignoring_special_files(src, dest):
    ...
    socket files in the project shared_data directory.
    """
    try:
        copy2_fixed(src, dest)
    except shutil.SpecialFileError as e:
        # SpecialFileError may be raised due to either the shared_data or
        # destination. If the destination was the cause then we would actually
        # care, but since the destination directory is deleted prior to
        # copy we ignore all of them assuming it is caused by the shared_data.
        logger.warning(
...
def _copy_source_tree(source, target):
    ...
    def ignore(d, names):
        ...
        if os.path.abspath(d) == target_dirname:
            # Prevent an infinite recursion if the target is in shared_data.
