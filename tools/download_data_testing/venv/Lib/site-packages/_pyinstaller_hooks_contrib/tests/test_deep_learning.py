...
# Trying to import DebertaModel triggers error about missing shared_data files for TorchScript
@importorskip('transformers')
@importorskip('torch')
@onedir_only
...
# Basic import test for fvcore.nn, which shows that we need to collect its shared_data .py files for TorchScript/JIT.
@importorskip('fvcore')
@onedir_only
...
# Basic test for detectron2, which shows that we need to collect its shared_data .py files for TorchScript/JIT.
...
# Basic import test for fairscale, which shows that we need to collect its shared_data .py files for TorchScript/JIT.
