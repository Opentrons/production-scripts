...
Duro server access should preferably use the API key in `backend/.env`:
...
The default CDP endpoint is `http://127.0.0.1:9222` and its persistent profile is stored under the git-ignored `backend/auth/duro-chrome-profile`. Configure the backend with:
