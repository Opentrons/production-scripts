...
def get_duro_products(
    refresh: bool = Query(default=False, description="忽略五分钟缓存并重新读取 Duro"),
) -> DuroProductSearchResponse:
...
def get_duro_product_bom(
    product_id: str,
    refresh: bool = Query(default=False, description="忽略五分钟缓存并重新读取产品 BOM"),
) -> DuroProductBomResponse:
    try:
...
def get_duro_component_children(
    component_id: str,
    refresh: bool = Query(default=False, description="忽略五分钟缓存并重新读取子组件"),
) -> DuroComponentChildrenResponse:
    try:
