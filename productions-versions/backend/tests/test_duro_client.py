...
def test_search_products_sends_bearer_token_and_browser_payload(
    ...
    monkeypatch.delenv("DURO_API_KEY", raising=False)
...
def test_rejected_explicit_token_retries_with_remote_chrome(
    ...
    monkeypatch.setenv("PRODUCTIONS_VERSIONS_DURO_TOKEN", explicit_token)
    provider = FakeBrowserTokenProvider(browser_token)
...
def test_api_key_is_used_when_no_temporary_access_token_is_exported(
    ...
    monkeypatch.delenv("PRODUCTIONS_VERSIONS_DURO_TOKEN", raising=False)
    monkeypatch.delenv("DURO_API_KEY", raising=False)
...
def test_explicit_access_token_overrides_api_key(
    ...
    monkeypatch.setenv("PRODUCTIONS_VERSIONS_DURO_TOKEN", access_token)
    monkeypatch.delenv("DURO_API_KEY", raising=False)
    session = FakeSession()
    client = DuroClient(
        api_key=api_key,
